...
class MergedClassifier(nn.Module):
    def __init__(self,h_dims,out_dims,model_type='bert'):
        ...
        if model_type=='bert':
            ...
            # Instantiate BERT model
            self.model = RobertaModel.from_pretrained('roberta-base')
        D_in, H, D_out = 768+6, self.h_dims, self.out_dims
        self.classifier = nn.Sequential(
            nn.Linear(D_in, H),
            nn.ReLU(),
            #nn.Dropout(0.5),
...
class FinetunedClassifier(nn.Module):
    """Finetuning classification model with external features.
    """
    def __init__(self,h_dims,out_dims,emo_dims):
        ...
        # Instantiate an one-layer feed-forward classifier
        # self.fc1 = pretrained_model.classifier[0]
        self.fc1 = nn.Linear(D_in, H)
        self.fc2 = nn.Linear(H + self.emo_dims, D_out)
       
        
    def forward(self, input_ids, attention_mask,x):
        """
        Feed pretrained roberta model to finetune with extra features.
        @param    input_ids (torch.Tensor): an input tensor with shape (batch_size,
                      max_length)
        @param    attention_mask (torch.Tensor): a tensor that hold attention mask
                      information with shape (batch_size, max_length)
        ...
        last_hidden_state_cls = outputs[0][:, 0, :]
        out1 = self.fc1(last_hidden_state_cls)
...
class UserAugmentedClassifier(nn.Module):
    def __init__(self, usermodel,h_dims,out_dims):
